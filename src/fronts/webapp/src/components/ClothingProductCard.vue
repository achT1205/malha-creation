...
selectedColor.value = props.product.colorVariants[0]
...
<template>
  <div class="group relative cursor-pointer" v-if="selectedColor">
    <div class="aspect-h-1 aspect-w-1 w-full overflow-hidden bg-gray-200">
      <img :src="selectedColor.image.imageSrc" :alt="selectedColor.image.altText"
        class="h-full w-full object-cover object-center group-hover:opacity-75" />
    </div>
    <div class="absolute inset-x-0 top-0 flex h-4/5 items-end justify-start overflow-hidden rounded-lg p-4"
        ...
        <div class="px-4 py-2">
          <div class="mt-1">
            ...
            <RadioGroup v-model="selectedColor.selectedSize" class="mt-4 grid grid-cols-6 gap-2">
              <RadioGroupOption as="template" v-for="sizeVariant in selectedColor.sizeVariants" :key="sizeVariant.size"
                :value="sizeVariant" :disabled="!sizeVariant.quantity || sizeVariant.quantity == 0"
                v-slot="{ active, checked }" @click="onSelectSize($event, sizeVariant)">
                <div :class="[
                   sizeVariant.inventoryStatus !== 'OUTOFSTOCK'
                  ...
                  sizeVariant.size.length > 3 ? 'w-12' : ''
                ]">
                  <span>{{ sizeVariant.size }}</span>
                  <span v-if=" sizeVariant.inventoryStatus !== 'OUTOFSTOCK'" :class="[
    ...
    <div class="mt-6 text-center">
      ...
      <RadioGroup v-model="selectedColor" class="mt-auto flex items-center justify-center space-x-3 pt-6">
        <RadioGroupOption as="template" v-for="colorVariant in props.product.colorVariants" :key="colorVariant.color"
          :value="colorVariant" :aria-label="colorVariant.color" v-slot="{ active, checked }">
          <div :class="[
            colorVariant.class,
            (active && checked) || selectedColor.color === colorVariant.color ? 'ring ring-offset-1' : '',
            !active && checked ? 'ring-2' : '',
            'relative -m-0.5 flex cursor-pointer items-center justify-center rounded-full p-0.5 ring-current focus:outline-none'
          ]">
            <span aria-hidden="true" class="h-3 w-3 rounded-full border border-black border-opacity-10 bg-current" />
          </div>
        </RadioGroupOption>
      </RadioGroup>
      ...
      <p class="text-gray-900 mt-auto flex items-center justify-center space-x-3 pt-6">
        <span :class="product.discount
            ...
            : ''
          ">{{ selectedColor.sizeVariants[0].price }} €
        </span>
        <span v-if="product.discount">
          {{ selectedColor.sizeVariants[0].discountedPrice }} €
        </span>
      </p>
    </div>
  </div>
